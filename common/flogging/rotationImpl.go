...
var _ io.WriteCloser = (*RotateLogger)(nil)
...
const (
	rotate_size  = RotateType("S")
	rotate_daily = RotateType("D")
)
...
type RotateLogger struct {
	rotatetype RotateType `json:"rotatetype" yaml:"rotatetype"`
...
func (l *RotateLogger) schedulling() {
	scheduler.Every().Day().At("00:00").Run(l.task)
}

func (l *RotateLogger) task() {
	l.mu.Lock()
	defer l.mu.Unlock()
	l.rotate()
}
...
func (l *RotateLogger) openExistingOrNew(writeLen int) error {
	l.mill()
...
// filename.
func (l *RotateLogger) prefixAndExt() (prefix, ext string) {
